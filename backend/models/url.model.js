...
function saveUrl(originalUrl, shortId, userId) {
  const savingUrl = db.prepare(
    "INSERT INTO urls (originalUrl, shortId, userId) VALUES (?, ?, ?)"
  );
  savingUrl.run(originalUrl, shortId, userId);
}

function getUrl(shortId) {
  const gettingUrl = db.prepare(
...
function updateShortId(shortId, newShortId) {
  const updatingShortId = db.prepare(
    "UPDATE urls SET shortId = ? WHERE shortId = ?"
  );
